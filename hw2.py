...
def fight(participant1, participant2, first2attack):
    ...
    ######################
    rounds  = 0
    # HP is the 3rd stored index
    
    variable1 = participent1[2]
    variable2 = participent2[2]

    while variable1 > 0 and variable2 > 0:
        if first2attack == 1:
            attMult = attack_multiplier(participent1[1], participent2[1])
            attDamage = variable1 * participent1[3]
            variable2 = variable2 - attDamage
        else:
            attMult = attack_multiplier(participent1[1], participent2[1])
            attDamage = variable2 * participent2[3]
            variable1 = variable1 - attDamage
...
def tournament(participants):
    ...
    # 3. Return WINS list
    ######################
#setting up list and indexes for the fight
    wins = [0 for i in range(len(participents))]
    index1 = 0
    index2 = 0
# calling fight function
    for participent in participents:
        home = fight(participent1, participent2, 1)
        away = fight(participent1, participent2, 2)
# checking the results (home)
        if home[0] == 1:
            wins[index1] += 1
        else:
            wins[index2] += 1
# checking the results (away)
        if away[0] == 1:
            wins[index1] += 1
        else:
            wins[index2] += 1
